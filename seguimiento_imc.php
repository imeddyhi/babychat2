...
// Obtén el email del padre que ha iniciado sesión
$email_padre = $_SESSION['email'];

// Verificar si se ha enviado un ID de bebé a través del formulario de búsqueda
$id_bebe = null;
if (isset($_POST['id_bebe']) && !empty($_POST['id_bebe'])) {
    $id_bebe = intval($_POST['id_bebe']); // Convertir a entero para seguridad
} elseif (isset($_GET['id_bebe'])) {
    $id_bebe = intval($_GET['id_bebe']);
}

// Consulta para obtener el historial de IMC del bebé si hay un ID válido
if ($id_bebe) {
    $query_imc = "SELECT id_imc, imc, fecha, id_bebe FROM seguimiento_imc WHERE id_bebe = ?";
    $stmt_imc = $conn->prepare($query_imc);
    $stmt_imc->bind_param('i', $id_bebe);
    $stmt_imc->execute();
...
<head>
    ...
    <title>Historial de IMC del Bebé</title>
    <style>
        body {
...
<body>
    <main>
        <div class="box">
            <div class="header">
                <h2 class="header-title">Historial de IMC del Bebé</h2>
                ...
                <a href="#" onclick="agregarIMC()"><button>+ Agregar</button></a>
            </div>

            <?php if ($id_bebe && isset($result_imc) && $result_imc->num_rows > 0): ?>
                <table class="table">
                    <thead>
                        <tr>
                            <th>IMC</th>
                    ...
                    <tbody>
                        <?php while($row = $result_imc->fetch_assoc()): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($row['imc']); ?></td>
                                <td><?php echo htmlspecialchars($row['fecha']); ?></td>
                                <td>
                                    <a href="imc_editar.php?id_imc=<?php echo htmlspecialchars($row['id_imc']); ?>&id_bebe=<?php echo htmlspecialchars($row['id_bebe']); ?>"><button class="edit">Editar</button></a>
                                </td>
                                <td>
                                    <a href="imc_eliminar.php?id_imc=<?php echo htmlspecialchars($row['id_imc']); ?>&id_bebe=<?php echo htmlspecialchars($row['id_bebe']); ?>"><button class="delete">Eliminar</button></a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
            ...
            <?php elseif ($id_bebe): ?>
                <p>No se encontraron registros de IMC para este bebé.</p>
            <?php endif; ?>
    ...
    <script>
        ...
        function agregarIMC() {
            ...
            if (idBebe) {
                window.location.href = `imc_agregar.php?id_bebe=${idBebe}`;
            } else {
