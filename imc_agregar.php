...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query_insertar = "INSERT INTO seguimiento_imc (id_bebe, imc, fecha) VALUES (?, ?, ?)";
    $stmt_insertar = $conn->prepare($query_insertar);
    $stmt_insertar->bind_param('ids', $id_bebe, $imc, $fecha);
    $stmt_insertar->execute();

    // Redireccionar para evitar reenvío del formulario
    header("Location: seguimiento_imc.php?id_bebe=" . $id_bebe);
    exit();
}
?>
...
<head>
    ...
    <title>Agregar IMC del Bebé</title>
    <style>
        body {
...
<body>
    <div class="form-container">
        <h2>Agregar IMC del Bebé</h2>
        <form method="post" action="">
            <input type="hidden" name="id_bebe" value="<?php echo htmlspecialchars($id_bebe); ?>">
            <input type="number" step="0.01" name="imc" placeholder="imc" required>
